...
export const isName = (fullName = "", allVal, props) => {
  ...
  if (maxLength(value.trim(), MAX_FULL_NAME_LENGTH))
    return `The maxim length of Name can be ${MAX_EMAIL_LENGTH}`;
};
...
export const isEmail = (email = "", allVal, props) => {
  ...
  if (maxLength(email, MAX_EMAIL_LENGTH))
    return `The maxim length of email can be ${MAX_EMAIL_LENGTH}`;
};
